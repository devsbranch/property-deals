...
def get_one_property(prop_id):
    ...
    if not prop_data:
        return jsonify({"message": f"The property listing with ID {prop_id} was not found."})
...
def delete_user(prop_id):
    ...
    db.session.commit()
    return jsonify({"message": f"The property listing '{prop_to_delete.name}' has been deleted."})
